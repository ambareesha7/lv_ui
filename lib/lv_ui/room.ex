defmodule LvUi.Room do
  ...
  alias LvUi.{Peer, PeerSupervisor}
  alias LvUiWeb.PeerChannel
  ...
  @spec add_peer(pid(), String.t()) :: {:ok, Peer.id()} | {:error, :peer_limit_reached}
  def add_peer(channel_pid, user_id) do
    GenServer.call(__MODULE__, {:add_peer, channel_pid, user_id})
  end
  ...
  @impl true
  def handle_call({:add_peer, _channel_pid, _user_id}, _from, state)
  ...
  @impl true
  def handle_call({:add_peer, channel_pid, user_id}, _from, state) do
    id = user_id
    # id = generate_id()
